...
public class CodeGenerator {
	// 演示例子，执行 main 方法控制台输入模块表名回车自动生成对应项目目录中
	    ...
	    public static void main(String[] args) {
	        ...
	        gc.setOutputDir("E:/dev3/workspace/simple-auth/src/main/java");	//生成文件的输出目录
...
//	        gc.setEntityName("%sEntity");			//实体命名方式  默认值：null 例如：%sEntity 生成 UserEntity
	        ...
	        dsc.setUrl("jdbc:mysql://localhost:3306/simple-auth?useUnicode=true&useSSL=false&characterEncoding=utf8");
...
//	        strategy.setInclude(scanner("表名"));		//需要包含的表名，允许正则表达式（与exclude二选一配置）
	        strategy.setInclude(new String[] { "sys_user" }); // 需要生成的表可以多张表
